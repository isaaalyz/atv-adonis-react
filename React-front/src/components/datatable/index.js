...
export default function DataTable({ data = [], setData, remove, ...props }) {
          ...
          {(data || []).map((element, index) => (
            <tr key={index}>
              {props.keys.map((key, i) => (
                props.hide[i]
                  ? <td className='d-none d-md-table-cell' key={i}>{element[key]}</td>
                  : <td key={i}>{element[key]}</td>
              ))}
              <td>
                {permissions[props.crud[2]] && ( // EDIT
                ...
                {permissions[props.crud[3]] && remove && ( // DELETE
